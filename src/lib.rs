...
impl HttpApiProblem {
    ...
    pub fn to_hyper_response(self) -> hyper::Response<hyper::Body> {
        ...
        parts.headers.insert(CONTENT_TYPE, HeaderValue::from_static(PROBLEM_JSON_MEDIA_TYPE));
        parts.headers.insert(CONTENT_LENGTH, HeaderValue::from_str(&length.to_string()).unwrap());
